...
namespace NuGet.Packaging.Core
{
    public class PackageDependencyComparer : IEqualityComparer<PackageDependency>
    {
        ...
        public bool Equals(PackageDependency x, PackageDependency y)
        {
            ...
            if (result)
            {
                result = x.Include.OrderedEquals(y.Include, s => s, StringComparer.OrdinalIgnoreCase);
            }

            if (result)
            {
                result = x.Exclude.OrderedEquals(y.Exclude, s => s, StringComparer.OrdinalIgnoreCase);
            }
